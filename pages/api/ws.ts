...
const code2uid = new Map<string, string>();
...
// ----- ユーティリティ -----
function send(ws: any, msg: any){ try{ ws.send(JSON.stringify(msg)); }catch{} }
...
function broadcastRoom(roomId: string, msg: any){ const r = rooms.get(roomId); if(!r) return; for(const uid of r.members){ const s = sockets.get(uid); if(s && s.readyState === 1) send(s, msg); } }
...
export default function handler(req: IncomingMessage, socket: Socket, head: Buffer) {
  if (!wss) {
    wss = new Server({ noServer: true });
    
    wss.on('connection', (ws: Sock) => {
...
function handleMessage(server: Sock, data: any) {
  // --- hello ---
  if (data.type === 'hello') {
  ...
  if (data.type === 'create-room') {
    ...
    if (room.isPublic) {
      sockets.forEach(s => { if (s.readyState === 1) send(s, { type: 'rooms', rooms: publicRoomsPayload() }); });
    } else {
...
function handleDisconnect(server: Sock) {
  const uid = server.uid!;
  sockets.delete(uid);
